                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('admin.variants.update', $variant) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="variant_name" class="block mt-1 w-full" type="text" name="variant_name" :value="old('variant_name', $variant->variant_name)" required />
                        ...
                        <div class="mb-4">
                            ...
                            <select id="product_id" name="product_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                ...
                                @foreach($products as $product)
                                    <option value="{{ $product->product_id }}" {{ old('product_id', $variant->product_id) == $product->product_id ? 'selected' : '' }}>{{ $product->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="stock_quantity" class="block mt-1 w-full" type="number" name="stock_quantity" :value="old('stock_quantity', $variant->stock_quantity)" required min="0" />
                            <x-input-error :messages="$errors->get('stock_quantity')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="price" :value="__('Price')" />
                            <x-text-input id="price" class="block mt-1 w-full" type="number" name="price" :value="old('price', $variant->price)" required min="0" step="0.01" />
