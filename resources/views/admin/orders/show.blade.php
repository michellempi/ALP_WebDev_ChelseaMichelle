                    ...
                    <div class="mb-6">
                        <div class="flex justify-between items-center">
                            ...
                            <form method="POST" action="{{ route('admin.orders.update-status', $order) }}" class="flex items-center space-x-2">
                                @csrf
                                @method('PATCH')
                                <select name="status_id" class="px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    @foreach($orderStatuses as $status)
                                        <option value="{{ $status->order_status_id }}" {{ $order->order_status_id == $status->order_status_id ? 'selected' : '' }}>
                                            {{ $status->name }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg">
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <p><strong>Role:</strong> {{ $order->user->role->name }}</p>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <div class="bg-gray-50 p-4 rounded-lg">
                                ...
                                @else
                                    <p class="text-yellow-600"><strong>No payment proof uploaded</strong></p>
                                @endif
                            </div>
                        </div>
                    </div>
